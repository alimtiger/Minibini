...
class PurchaseOrderModelTest(TestCase):
    ...
    def test_purchase_order_creation(self):
        po = PurchaseOrder.objects.create(
            job_id=self.job,
            price_list_item_id="PLI001",
            po_number="PO001"
        )
        self.assertEqual(po.job_id, self.job)
        self.assertEqual(po.price_list_item_id, "PLI001")
    ...
    def test_purchase_order_optional_job(self):
        po = PurchaseOrder.objects.create(
            po_number="PO003",
            price_list_item_id="PLI002"
        )
        self.assertIsNone(po.job_id)
        self.assertEqual(po.price_list_item_id, "PLI002")
    ...
    def test_purchase_order_optional_price_list_item(self):
        ...
        self.assertEqual(po.job_id, self.job)
        self.assertEqual(po.price_list_item_id, "")
...
class BillModelTest(TestCase):
    def setUp(self):
        ...
        self.job = Job.objects.create(
            job_number="JOB001",
            contact_id=self.customer_contact,
            description="Test job"
        )
    ...
    def test_bill_creation(self):
        ...
        self.assertEqual(bill.po_id, self.purchase_order)
        self.assertEqual(bill.contact_id, self.contact)
    ...
    def test_bill_with_contact_deletion(self):
        ...
        # Bill should be deleted due to CASCADE
        with self.assertRaises(Bill.DoesNotExist):
            Bill.objects.get(po_id=self.purchase_order, vendor_invoice_number="VIN004")