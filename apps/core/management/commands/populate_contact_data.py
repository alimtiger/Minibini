...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Load all fixtures from contact_test_data directory
        fixtures_dir = os.path.join('fixtures', 'contact_test_data')
        if not os.path.exists(fixtures_dir):
            self.stdout.write(self.style.WARNING(f'Fixtures directory {fixtures_dir} does not exist yet. No data to load.'))
            return
        ...
        if not fixture_files:
            self.stdout.write(self.style.WARNING('No fixture files found in contact_test_data directory.'))
            return

        self.stdout.write(f'Loading {len(fixture_files)} fixture files...')

        for fixture_file in fixture_files:
            fixture_path = os.path.join('fixtures', 'contact_test_data', fixture_file)
        ...
        self.stdout.write('\n' + self.style.SUCCESS('Contact test data populated successfully!'))
        self.stdout.write('\nDatabase summary:')
